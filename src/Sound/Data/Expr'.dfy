module Expr' {
    ...
    | UnaryMinus
  {
    ...
    predicate IsBinaryOperator() {
      match this
      case Equiv | LogicalImp | LogicalAnd | LogicalOr | Eq | Neq | Less | AtMost => true
      case _ => false
    }
        ...
        if op == IfThenElse then
          ...
          if |ss| != 2 then None
            ...
            match (e0, e1, op) {
              ...
              case (v0, v1, Eq) => Some(BVal(v0 == v1))
              case (v0, v1, Neq) => Some(BVal(v0 != v1))
  ...
  function Eq(e1: Expr, e2: Expr): Expr {
    OperatorExpr(Operator.Eq, [e1, e2])
  }
