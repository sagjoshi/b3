...
//     }
  ...
  newtype CallArguments = seq<CallArgument> {
    ...
    function Vars(): set<Variable> {
      if this == [] then {} else {this[0].v} + this[1..].Vars()
    }
...
//     }
    ...
    | Call(proc: Procedure, args: CallArguments)
  {
    ...
    predicate ValidCalls() {
      match this
      case Call(proc, args) =>
        && (forall e <- proc.Pre :: e.IsDefinedOn(args.Vars()))
        && (forall e <- proc.Post :: e.IsDefinedOn(args.Vars() /*+ args.NumInOutArgs()*/))
    ...
    function FVars(): set<Variable> {
      ...
      case Call(proc, args) => args.Vars() /*+ args.NumInOutArgs()*/
    }
