...
//     function Upd0(): VarsJumps {
//       if 0 in Keys then this else map[0 := {}]
//     }

//     function Merge(m1: VarsJumps): VarsJumps {
//       var s0 := if 0 in Keys then this[0] else {};
//       var s1 := if 0 in m1.Keys then m1[0] else {};
...
//       map i: nat | 
//         && i <= Max'(Keys) + 1
//         && Pred(i) in Keys :: SubstractSet(n, this[Pred(i)]) 
//     }

//     lemma SubstractLemma(n: nat, i: nat)
//       requires Pred(i) in Keys
//       ensures Substract(n)[i] == SubstractSet(n, this[Pred(i)])
//     {

//     }

//     // predicate InUpto(i:idx, n: nat) 
//     //   requires n in Keys
//     // {
...
//           && st' in posts[k]
//           && |st'| == |st|
//           && forall i: Idx :: i < |st| && !(k in Keys && i in this[k]) ==> st[i] == st'[i]
//       )
//     }
//   }
...
//       var vs := Process'(s);
//       vs.Substract(n)
//     case Escape(n) => map[n := {}, 0 := {}]
//     case Loop(inv, body) => Process'(body)[0 := {}]
...
//   lemma Process'CorrectAux(stmt: Stmt, m: VarsJumps) 
//     requires Process'(stmt) == m
//     ensures forall v <- m.Values, s <- v :: s < stmt.Depth()
//     ensures 0 in m.Keys
//     ensures forall k <- m.Keys :: m[0] <= m[k]
//   {
//     match stmt 
//     case Choice(s0, s1) =>
//       var m0 := Process'(s0);
//       var m1 := Process'(s1);
//       Process'CorrectAux(s0, m0);
//       Process'CorrectAux(s1, m1);
//     case Loop(_, body) => 
//     case NewScope(n, s) => 
//       var m' := Process'(s);
//       Process'CorrectAux(s, m');
//     case Seq(ss) => SeqProcess'CorrectAux(ss, m);
//     case _ => 
//   }

//   lemma SeqProcess'CorrectAux(ss: seq<Stmt>, m: VarsJumps) 
...
//       var m' := Process'(body);
//       var stvar := set i: Idx | i < |st|;
//       Process'CorrectAux(body, m');
//       var inv' := inv * m'.ToEqs(st, posts.UpdateHead(inv)).head;
//       assert st in inv';
//       forall st': State | st' in inv' ensures Omni.Sem(body, st', (m'[0 := {}]).ToEqs(st, posts).UpdateHead(inv')) {
//         Omni.SemCons(body, st', m'.ToEqs(st', posts.UpdateHead(inv)), (m'[0 := {}]).ToEqs(st, posts).UpdateHead(inv')) by {
//           Process'Correct(body, st', m', posts.UpdateHead(inv)) by {
//             assert Omni.PreservesInv(inv, body, posts);
//           }
//         }
//       }
//     case NewScope(n, s) => assume false;
