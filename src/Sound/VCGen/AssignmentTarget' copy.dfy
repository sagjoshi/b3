module AssignmentTarget'' {
  ...
  newtype VarsJumps = map<nat, set<Idx>> {
    ...
    opaque function Substract(n: nat): VarsJumps 
      ensures forall i: Idx :: i + 1 in Keys ==> i in Substract(n).Keys
      ensures 0 in Keys ==> 0 in Substract(n).Keys
      ensures 0 in Substract(n).Keys ==> Substract(n)[0] == SubstractSet(n, this.Get(0) + this.Get(1))
      ensures forall i: Idx :: (i + 1 in Keys && i != 0) ==> Substract(n)[i] == SubstractSet(n, this[i + 1])
    {
      var m: VarsJumps := map i: nat | 
        && i <= Max'(Keys) + 1
        && Trigger(i)
        && i + 1 in Keys :: SubstractSet(n, this[i + 1]);
      // var s0 := SubstractSet(n, if 0 in Keys then this[0] else {});
    ...
    // lemma SubstractLemma(n: nat, i: Idx)
    //   ensures i + 1 in Keys ==> i in Substract(n).Keys
    //   ensures 0 in Substract(n).Keys ==> Substract(n)[0] == SubstractSet(n, this.Get(0) + this.Get(1))
    //   ensures (i + 1 in Keys && i != 0) ==> Substract(n)[i] == SubstractSet(n, this[i + 1])
    // {
